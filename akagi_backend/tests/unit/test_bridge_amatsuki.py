...
def test_handle_draw(bridge):
    ...
    stomp = make_stomp(f"{'/user/topic/desk/draw/'}0", content)
...
def test_handle_tehai_action_kiri(bridge):
    ...
    stomp = make_stomp(f"{'/topic/desk/tehaiAction/'}.0", content)
...
def test_handle_river_action_pon(bridge):
    ...
    stomp = make_stomp(f"{'/topic/desk/riverAction/'}.0", content)
...
def test_handle_join_desk_callback(bridge) -> None:
    ...
    stomp = make_stomp("/user/topic/callback/joinDesk", content)
...
def test_handle_round_start_happy_path(bridge):
    ...
    stomp = make_stomp("/user/topic/desk/roundStart/" + "1", content)
...
def test_handle_join_desk_callback_failures(bridge):
    ...
    # 状态错误
    bridge.valid_flow = False
    stomp = make_stomp("/user/topic/callback/joinDesk", {"status": 1, "errorCode": 0})
    bridge._handle_join_desk_callback(stomp)
    assert bridge.valid_flow is False

    # 错误码错误
    bridge.valid_flow = False
    stomp = make_stomp("/user/topic/callback/joinDesk", {"status": 0, "errorCode": 1})
    bridge._handle_join_desk_callback(stomp)
    assert bridge.valid_flow is False

    # 游戏类型错误
    bridge.valid_flow = False
    stomp = make_stomp("/user/topic/callback/joinDesk", {"status": 0, "errorCode": 0, "gameType": 1})
    bridge._handle_join_desk_callback(stomp)
    assert bridge.valid_flow is False

    # 游戏模式错误
    bridge.valid_flow = False
    stomp = make_stomp("/user/topic/callback/joinDesk", {"status": 0, "errorCode": 0, "gameType": 0, "gameMode": 2})
    bridge._handle_join_desk_callback(stomp)
    assert bridge.valid_flow is False
